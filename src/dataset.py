...
class SynthRADDatasetOnTheFly(Dataset):
    def __init__(self, patient_ids, anatomy, data_root,
        ...
        for pid in tqdm(patient_ids, desc=f"Indexing {anatomy} {split}"):
            ...
            try:
                mask_array = self._load_mha(mask_path)
    ...
    def __getitem__(self, idx):
        ...
        try:
            mr_array   = self._load_mha(os.path.join(patient_path, "mr.mha"))
            ct_array   = self._load_mha(os.path.join(patient_path, "ct.mha"))
            mask_array = self._load_mha(os.path.join(patient_path, "mask.mha"))
        except Exception as e:
            print(f"❌ SKIPPING bad sample → Patient: {patient_id} | Slice: {slice_idx} | {e}")
...
def make_dataloader(patient_ids, anatomy, data_root,
    ...
    return DataLoader(
        ...
        pin_memory         = True,
        timeout            = 120,        # raises exception instead of hanging
