...
from src.models import ResNetGenerator, PatchGANDiscriminator, MultiScaleDiscriminator, init_weights, ImageBuffer
...
def train_step(G, F, D_CT, D_MR, opt_G, opt_D,
    ...
    with autocast():
        ...
        pred_fake_MR1, pred_fake_MR2 = D_MR(fake_MR)   # multi‑scale returns two tensors

        loss_G = (
            gan_loss(pred_fake_CT, torch.ones_like(pred_fake_CT))
            + gan_loss(pred_fake_MR1, torch.ones_like(pred_fake_MR1))
            + gan_loss(pred_fake_MR2, torch.ones_like(pred_fake_MR2))   # ✅ FIXED: added second scale
    ...
    # Get one buffered image for each discriminator (do NOT push again inside autocast)
    ...
    with autocast():
        ...
        # Use the already buffered images (not a second push)
        pred_fake_CT2       = D_CT(fake_CT_buf)
        pred_fake_MR_buf1, pred_fake_MR_buf2 = D_MR(fake_MR_buf)
...
def validate(G, F, val_loader, device):
    ...
    # Grad enabling is automatically restored after 'with torch.no_grad()'
    G.train()
    F.train()

    return {
        "mr2ct_ssim": np.mean(mr2ct_ssim),
        "ct2mr_ssim": np.mean(ct2mr_ssim),
        "mr2ct_mae":  np.mean(mr2ct_mae),
        "ct2mr_mae":  np.mean(ct2mr_mae),
    }
...
def main():
    ...
    if existing:
        ...
        D_CT.load_state_dict(ckpt["D_CT"])
        D_MR.load_state_dict(ckpt["D_MR"])
        ...
        if os.path.exists(log_path):
            ...
            best_ssim = max((v1 + v2) / 2 for v1, v2 in zip(log["mr2ct_ssim"], log["ct2mr_ssim"])) if log["mr2ct_ssim"] else -1.0
    ...
    for epoch in range(start_epoch, config["EPOCHS"] + 1):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch}/{config['EPOCHS']}"):
            losses = train_step(G, F, D_CT, D_MR, opt_G, opt_D,
                                batch, gan_loss, l1_loss, buf_CT, buf_MR,
                                config, device, scaler_G, scaler_D, perceptual_loss)
            if losses is None:          # ✅ skip batch if NaN occurred
                continue
